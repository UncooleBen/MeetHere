...
A standalone .css file (including necessary dropdown styles and alternative, text-based arrows) can be generated by running `buildng/build_standalone.less` through the `lessc` compiler:

```bash
$ lessc buildng/build_standalone.less datetimepicker.css
```
...
The increment used to buildng the hour view. A button is created for each <code>minuteStep</code> minutes.
